  ...
  def destroy
    if @request.user.id == current_user.id
      if @request.status == 'accepted'
        render json: {"message": "This request was already accepted by the item owner. Can't withdraw accepted request"}, status: :unprocessable_entity
      elsif @request.status == 'declined'
        render json: {"message": "This request was declined by the item owner. Can't withdraw declined request"}, status: :unprocessable_entity
      ...
      render json: {"message": "You can only destroy requests that belong to you"}, status: :unprocessable_entity
    end
  end
  ...
  def decline
    ...
    if current_user.id == @request.item.user.id
      if @request.status == 'declined'
        render json: {"message": "You have already declined this request"}, status: :unprocessable_entity
      else  
        @request.update_attributes(:status => 'declined')
        render json: {"message": "Successfully Declined"}, status: :ok
      end
    else
      render json: {"message": "Item being requested does not belong to you do you can't decline the request"}, status: :unprocessable_entity
    end
  end
